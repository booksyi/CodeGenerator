...
{
    public class GenerateActionReads
    {
        ...
        public class Handler : IRequestHandler<Request, ResponseBase<bool>>
        {
            ...
            public async Task<ResponseBase<bool>> Handle(Request request, CancellationToken token)
            {
                ...
                string template = await File.ReadAllTextAsync(@"Templates\Api\Mediator\Actions\Reads.txt");
                ...
                await File.WriteAllTextAsync(Path.Combine(request.OutputPath, $"Get{plural.UpperFirst()}.cs"), code);
