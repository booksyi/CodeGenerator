...
{
    public class GenerateActionDelete
    {
        ...
        public class Handler : IRequestHandler<Request, ResponseBase<bool>>
        {
            ...
            public async Task<ResponseBase<bool>> Handle(Request request, CancellationToken token)
            {
                ...
                string template = await File.ReadAllTextAsync(@"Templates\Api\Mediator\Actions\Delete.txt");
                ...
                await File.WriteAllTextAsync(Path.Combine(request.OutputPath, $"Delete{singular.UpperFirst()}.cs"), code);
