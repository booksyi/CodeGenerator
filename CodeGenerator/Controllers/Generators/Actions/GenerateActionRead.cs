...
{
    public class GenerateActionRead
    {
        ...
        public class Handler : IRequestHandler<Request, ResponseBase<bool>>
        {
            ...
            public async Task<ResponseBase<bool>> Handle(Request request, CancellationToken token)
            {
                ...
                string template = await File.ReadAllTextAsync(@"Templates\Api\Mediator\Actions\Read.txt");
                ...
                await File.WriteAllTextAsync(Path.Combine(request.OutputPath, $"Get{singular.UpperFirst()}.cs"), code);
