    ...
    public class GenerateController
    {
        ...
        public class Handler : IRequestHandler<Request, ResponseBase<bool>>
        {
            ...
            public async Task<ResponseBase<bool>> Handle(Request request, CancellationToken token)
            {
                ...
                string template = await File.ReadAllTextAsync(@"Templates\Api\Mediator\Controller.txt");
                ...
                await File.WriteAllTextAsync(Path.Combine(request.OutputPath, $"{plural.UpperFirst()}Controller.cs"), code);
