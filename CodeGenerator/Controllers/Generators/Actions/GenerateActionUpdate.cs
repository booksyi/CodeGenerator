...
{
    public class GenerateActionUpdate
    {
        ...
        public class Handler : IRequestHandler<Request, ResponseBase<bool>>
        {
            ...
            public async Task<ResponseBase<bool>> Handle(Request request, CancellationToken token)
            {
                ...
                string template = await File.ReadAllTextAsync(@"Templates\Api\Mediator\Actions\Update.txt");
                ...
                await File.WriteAllTextAsync(Path.Combine(request.OutputPath, $"Update{singular.UpperFirst()}.cs"), code);
